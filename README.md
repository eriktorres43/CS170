...
# **Introduction:**
   ...
   The Eight puzzle is a 3x3 square puzzle where we are given tiles with numbers 1-8 and an empty tile. This given puzzle is then randomly arranged, where we must solve the puzzle by moving the given tiles through the empty tile until we reach the solution. Typically, the solution of the puzzle is ordered from 1-8 with the empty tile on the bottom furthest right, and in this case, it is. Rather than solving this problem physically, we have instead chosen to solve this problem using search algorithms learned throughout the course, which I have done in the language C++.
